...
public class Bottom_ShroomPot extends Block implements IBonemealable {
	public Bottom_ShroomPot(String key, int id) {
	...
	int ChangeTopID = BlockID + ID_Shroom + 1;

	int ChangeTopID_Flower = BlockID + ID_Shroom + 2;
	...
	public boolean onBlockRightClicked(World world, int x, int y, int z, EntityPlayer player, Side side, double xHit, double yHit) {
		...
		if (MyID == ChangeID && UpID == ChangeTopID_Fruit) {
			int Randy = world.rand.nextInt(6);
			if (Randy == 1 || Randy == 2) {
				world.dropItem(x, y, z, new ItemStack(Block.mushroomBrown, world.rand.nextInt(3) + 1)); // thanks luke /^.^ \  <-- evil face
			} else if (Randy == 3 || Randy == 4) {
				world.dropItem(x, y, z, new ItemStack(Block.mushroomRed, world.rand.nextInt(3) + 1)); // thanks luke /^.^ \  <-- evil face
			} else {
				world.dropItem(x, y, z, new ItemStack(Block.getBlock(BlockID - ID_WhiteMushroom), world.rand.nextInt(2) + 1)); // thanks luke /^.^ \  <-- evil face
			}
